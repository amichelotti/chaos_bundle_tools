...
do
    ...
    if [ -n $loglevel ];then
	$CHAOS_PREFIX/bin/testDataSetIO --direct-io-client-kv-param ZMQ_RCVTIMEO:-1 --dsname $dataset_name --wait 61 --points 0 --pointmax $maxsize --nerror $nerror --metadata-server $metadata_server --nthread $i --pointincr 2 --loop $loop --page $page_len --report $CHAOS_PREFIX/log/$csvprefix-$i.csv --log-on-file 1 --log-file $CHAOS_PREFIX/log/$csvprefix-$i.log --log-level debug
	exit_status=$?
    else
	$CHAOS_PREFIX/bin/testDataSetIO --nerror $nerror --direct-io-client-kv-param ZMQ_RCVTIMEO:-1 --wait 61 --dsname $dataset_name --points 0 --pointmax $maxsize --metadata-server $metadata_server --nthread $i --pointincr 2 --loop $loop --page $page_len --report $CHAOS_PREFIX/log/$csvprefix-$i.csv
	exit_status=$?
    fi
