...
echo "set multiplot layout 8,1 title '$mach on $1, loops $loop' font ',14'" >> testioMulti.gnuplot  
...
for i in $nthreads;do
    ...
    $CHAOS_PREFIX/bin/testDataSetIO --points 0 --pointmax 10000 --metadata-server $1 --nthread $i --pointincr 2 --loop $loop --report testDataSetIO-$host-$i.csv --log-on-file --log-file testDataSetIO-$host-$i.log --log-level debug
